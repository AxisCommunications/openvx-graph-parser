...
/**
 *  @file   standalone_test_threshold_example_strip.c
 *  @brief  Standalone test code for the graph "threshold_example".
 */
...
/* Include the generated graph create function */
#include "threshold_example_strip.h"
#define GRAPHNAME "threshold_example"
...
int main(int argc, char *argv[]) {
    ...
    /* Userdata object used to transport width and height information to graph create function.
       Here we also set a threshold value to use in threshold node */
    threshold_example_userdata_t userdata = {.ref_width = ref_width, .ref_height = ref_height, .ref_thresh = 80};

    /* Create the output VX-images */
    ...
    output_images[2] =  vxCreateImage(context, userdata.ref_width, userdata.ref_height, VX_DF_IMAGE_U8);

    /* Call the generated create function to actually create the nodes and populate the graph skeleton */
    bool create_status = threshold_example_create(context, graph_skeleton, input_images, output_images, &userdata);
